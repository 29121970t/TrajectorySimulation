...
%r(:,2) = oven_y;
%r(:,1) = slt_w .*(2* randn(N,1) -1);
%r(:,3) = slt_h .* (2* randn(N,1) -1) + slt_z;
...
u_prime(:,[1 2 3]) = u_prime(:,[1 3 2]);
    ...
    theta1 = (2 * rand(N, 1) - 1) * 0.0122173 / 2;
    alpha1 = (2 * rand(N, 1) - 1) * pi / 2;
    
    y_canonical = cos(theta1);
    p_canonical = sin(theta1);
    x_canonical = p_canonical .* cos(alpha1);
    z_canonical = p_canonical .* sin(alpha1);

    % Combine into a matrix of canonical vectors
    canonicalVectors = [x_canonical,y_canonical, z_canonical];

% Initialize speed distribution
vstart = 590;                               % [m/s] minimum speed
vend = 690;                                 % [m/s] minimum speed
vabs = (vend-vstart)* rand(N,1) + vstart;   % Uniform sample
%vinit = vabs .* canonicalVectors;                    % Initial velocities
